...
var $author$project$CollatzConjecture$calculateSteps = F2(
	function (steps, start) {
		calculateSteps:
		while (true) {
			var _v0 = A2($elm$core$Debug$log, 'steps', steps);
			var _v1 = A2($elm$core$Debug$log, 'start', start);
			if (start === 1) {
				return steps;
			} else {
				if (!A2($elm$core$Basics$modBy, 2, start)) {
					var $temp$steps = steps + 1,
						$temp$start = (start / 2) | 0;
					steps = $temp$steps;
					start = $temp$start;
					continue calculateSteps;
				} else {
					var $temp$steps = steps + 1,
						$temp$start = (3 * start) + 1;
					steps = $temp$steps;
					start = $temp$start;
					continue calculateSteps;
				}
...
var $author$project$CollatzConjecture$collatz = function (start) {
	return (start <= 0) ? $elm$core$Result$Err('Only positive numbers are allowed') : $elm$core$Result$Ok(
		A2($author$project$CollatzConjecture$calculateSteps, 0, start));
};
...
var $author$project$Test$Generated$Main1942510958$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		paths: _List_fromArray(
			['/Users/vincentdupont/Exercism/elm/collatz-conjecture/tests/Tests.elm']),
		processes: 8,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 405077899757658
	},
...
_Platform_export({'Test':{'Generated':{'Main1942510958':{'init':$author$project$Test$Generated$Main1942510958$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "/tmp/elm_test-184.sock";
