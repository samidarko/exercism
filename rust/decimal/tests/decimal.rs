...
fn test_eq() {
    ...
    for big in BIGS.iter() {
        assert_eq!(decimal(big), decimal(big));
    }
}
...
fn test_eq_vary_sig_digits() {
    assert_eq!(decimal("0"), decimal("0000000000000.0000000000000000000000"));
    assert_eq!(decimal("1"), decimal("00000000000000001.000000000000000000"));
}
